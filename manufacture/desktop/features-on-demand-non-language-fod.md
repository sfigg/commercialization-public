...
> [!NOTE]
> The Mixed Reality Feature on Demand has a large installation size, and also requires installing an additional data assets package, if an updated asset package is available. Updates to the FOD are available via regular Windows LCUs, but the data assets package is available as a separate download via the [Microsoft Update Catalog](http://www.catalog.update.microsoft.com). 
