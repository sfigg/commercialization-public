...
|IAT in Executable Section         |  The import address table (IAT), should not be an executable section of memory. </br>This issue occurs when the IAT, is located in a Read and Execute (RX) only section of memory. This means that the OS will not be able to write to the IAT to set the correct addresses for where the referenced DLL. </br>One way that this can occur is when using the /MERGE (Combine Sections) option in code linking. For example, if .rdata (Read-only initialized data) is merged with .text data (Executable code), it is possible that the IAT may end up in an executable section of memory.|
