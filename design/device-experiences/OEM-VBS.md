...
|HVCI-compatible drivers | Ensure all system drivers have been tested and verified to be compatible with HVCI. The Windows Driver Kit and Driver Verifier contain tests for driver HVCI compatibility. There are four steps to verify driver compatibility:<br><ol><li>Use Driver Verifier with the new Code Integrity compatibility checks enabled.</li><li>Run the Hypervisor Code Integrity Readiness Test in the Windows HLK.</li><li>Test the driver on a system with VBS and HVCI enabled. This step is imperative to validate the driver's behavior with HVCI, as static code analysis tools simply aren't capable of detecting all HVCI violations possible at runtime.</li><li>Use the Device Guard Readiness Tool.</li></ol> |
