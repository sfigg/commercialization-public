...
| Virtualization | 1.Systems must have a processor that supports Input-Output Memory Management Unit (IOMMU) device virtualization and all I/O devices must be protected by IOMMU/SMMU 2.Systems must also have virtual machine extensions with second level address translation (SLAT) 3.The presence of these hardware virtualization features must be unmasked and reported as supported by the system firmware, and these features must be available for the operating system to use  | *For IOMMU, the system must have Intel VT-d, AMD-Vi, or ARM64 SMMUs *For SLAT, the system must have Intel Vt-x with Extended Page Tables (EPT), or AMD-v with Rapid Virtualization Indexing (RVI) |
