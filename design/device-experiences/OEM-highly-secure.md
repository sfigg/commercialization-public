...
| Virtualization | Systems must have a processor that supports Input-Output Memory Management Unit (IOMMU) device virtualization and all I/O devices must be protected by IOMMU/SMMU. The presence of these hardware virtualization features must be unmasked and reported as supported by the system firmware, and these features must be available for the operating system to use. Systems must also have virtual machine extensions with second level address translation (SLAT). | For IOMMU, you'll need Intel VT-d, AMD-Vi, or ARM64 SMMUs . For SLAT, you'll need Intel VT-X2 with Extended Page Tables (EPT), or AMD-v with Rapid Virtualization Indexing (RVI) |
