...
-   <xref local="BKMK_HCK_SC_hR">Hardware requirements</b>

-   <xref local="BKMK_HCK_SC_sR">Software requirements</b>

-   <xref local="BKMK_HCK_SC_tC">Test computer configuration</b>
...
-   The smart card reader that you want to test.
    ...
    >[!NOTE]
    If the device supports universal serial bus (USB), you must have two test devices to run the USB Serial Number test. For more information, see <xref rid="p_hlk_test.0f2d5113-cf70-4cda-8afc-b7005d1e2739">USB Serial Number</b>.

    
-   One Personal Computer / Smart Card (PC/SC) workgroup test card (from card set 2).

    This card set can be purchased from the <xref hlink="http://go.microsoft.com/fwlink/?LinkId=228902">PC/SC website</b>. Test the product by using the smart cards that the PC/SC workgroup test card set includes. Do not include these smart cards with your test submission.
