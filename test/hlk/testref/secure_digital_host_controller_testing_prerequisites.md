...
Secure Digital I/O (SDIO) host controllers must comply with Peripheral Component Interconnect (PCI) 2.3 or later requirements for that interface. For PCI configuration registers and interface information, see the SD Host Controller Specification, Version 1.0, Appendix A, which is available from the <xref hlink="http://go.microsoft.com/fwlink/?LinkId=229850">SD Association website</b>.
...
-   One test computer that meets the Windows HLK prerequisites. See <xref rid="p_sxs_hlk.windows_hlk_prerequisites">Windows HLK Prerequisites</b> for more information.
