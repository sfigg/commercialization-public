...
-   <xref local="BKMK_HCK_WSD_hR">Hardware requirements</b>.

-   <xref local="BKMK_HCK_WSD_sR">Software requirements</b>.

-   <xref local="BKMK_HCK_WSD_tC">Test computer configuration</b>.
...
-   Two test computers. For testing networking capabilities, the computer that the WDS-compliant device test device is physically attached to is referred to as the System Under Test (SUT) computer and the other computer is referred to as the support computer.
    ...
    >[!NOTE]
    All computers must meet the Windows HLK requirements. For more information, see <xref rid="p_sxs_hlk.windows_hlk_prerequisites">Windows HLK Prerequisites</b>.
