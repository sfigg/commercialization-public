...
`DNSDomain` specifies the primary Domain Name System (DNS) domain to be searched for the name resolution. The DNS suffix is used by the DNS client for registrations and to assist with resolution if no [DNSSuffixSearchOrder](dnssuffixsearchorder-win7-microsoft-windows-dns-clientdnssuffixsearchorder.md) is configured.
