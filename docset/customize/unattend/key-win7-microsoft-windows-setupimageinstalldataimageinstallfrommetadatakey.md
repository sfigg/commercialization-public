...
`Key` specifies whether the image index, name, or description is used to specify the metadata for an image in a Windows® image (.wim) file. For information about using this setting, see [MetaData](metadata-win7-microsoft-windows-setupimageinstalldataimageinstallfrommetadata.md).
