...
-   The value for `Key` is added to the answer file as an attribute of the [Portrule](portrule-win7-microsoft-windows-networkloadbalancing-coreclustersclusterportrulesportrule.md) element. The attribute `wcm:keyValue` is used to identify each unique port rule. For example, you can specify three different port rules by using the `Key` values of **Portrule1**, **Portrule2**, and **Portrule3**.
