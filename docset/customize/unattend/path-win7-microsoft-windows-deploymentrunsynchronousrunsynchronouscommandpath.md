...
`Path` specifies the path and the name of the command to run. [RunSynchronous](runsynchronous-win7-microsoft-windows-deploymentrunsynchronous.md) commands run in the user context in the auditUser configuration pass and in the system context in the specialize configuration pass.
...
<td><p>Specifies the path and the file name of the command to run synchronously. The path can be a local path or a UNC path. If the path is a UNC path, the [Credentials](credentials-win7-microsoft-windows-deploymentrunsynchronousrunsynchronouscommandcredentials.md) must be specified. <em>Path</em> is a string with a maximum length of 259 characters.</p></td>
</tr>
</tbody>
</table>
...
[Microsoft-Windows-Deployment](microsoft-windows-deployment-win7-microsoft-windows-deployment.md) | [RunSynchronous](runsynchronous-win7-microsoft-windows-deploymentrunsynchronous.md) | [RunSynchronousCommand](runsynchronouscommand-win7-microsoft-windows-deploymentrunsynchronousrunsynchronouscommand.md) | **Path**
