...
`UILanguageFallback` specifies the language to use for resources that are not localized for the default system user interface (UI) ([UILanguage](uilanguage-win7-microsoft-windows-international-core-winpeuilanguage.md) setting).
...
You must specify this setting only if the language that [UILanguage](uilanguage-win7-microsoft-windows-international-core-winpeuilanguage.md) specifies is not fully localized and has more than one fallback language. In all other cases, this setting will be ignored.
