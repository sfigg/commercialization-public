...
|Basic |	Microsoft-Windows-LanguageFeatures-Basic-fr-fr-Package | None | Spell checking, text prediction, word breaking, and hyphenation if available for the language.<p>You must add this component before adding any of the following components. |
| Fonts	Microsoft-Windows-LanguageFeatures-Fonts-Thai-Package |	None | Fonts.<p>Required for some regions to render text that appears in documents. Example, th-TH requires the Thai font pack. To learn more, see Features On Demand V2 (Capabilities). |
