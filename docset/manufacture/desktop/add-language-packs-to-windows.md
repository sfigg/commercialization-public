...
-	If you install an update (hotfix, general distribution release [GDR], or service pack [SP]) that contains language-dependent resources prior to installing a language pack, the language-specific changes in the update won't be applied when you add the language pack. You'll need to reinstall the update to apply language-specific changes,. To avoid reinstalling updates, install language packs before installing updates.
