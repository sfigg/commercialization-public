...
This manageability of a connected storage device means the user can perform an end-to-end storage workflow using a common user interface such as PowerShell, or the File and Storage Services in a Server Management tool. This workflow must be achievable from a Windows Server vNext, through its Storage Management API (SMAPI) and an associated Storage Management Provider (SMP), to the connected storage device providing the services.
